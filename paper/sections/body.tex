...
Decades of research on end-to-end verifiable (E2E-V) elections have shown strong cryptographic guarantees can be realized for protecting the integrity of elections and the secrecy of voter ballots. In more recent years, pilot projects and deployments of E2E-V platforms have also demonstrated that these systems are usable and scalable to an appropriate degree. Despite these efforts, the vendors of systems for governmental elections continue to run unverifiable elections where the content of individual ballots can be determined by at least one trusted entity in the platform.

In this paper, we contribute to a broader discussion the cryptographic voting community has been having for some number of years: do we compromise our ideals and offer watered down systems that the industry might find more palatable if they offer tangbile security advantages over the status quo? We relate this to the term Realpolitik, coined in 19th century Germany to describe statecraft based on pragmatic calculations rather than ideology, and propose \textit{Realcryptik} to describe such a pragmatic approach to election verification.

Flavours of Realcryptik already permeates the research literature. Despite opposition to internet voting, Helios is perhaps the most used E2E-V voting technology. While researchers argue coercion resistance is essential for internet voting, Helios provides none. Scantegrity II was the first E2E-V system to see governmental use. It is built around the compromise that in a paper ballot system, it is ok for optical scanner to learn how the voter voted, side-stepping the complex ballot mechanics its predecessors to avoid this. In recent years, serious discussions have arose around whether E2E-V should be largely abandoned in favour of risk limiting audits? Should we affirm the use of ballot marking devices when we know most voters do not carefully check their output?

We propose a specific compromise in this paper: let EAs and their vendors see the raw ballots, while protecting them from all outsiders (receipt-freeness). For the EA, we still rely on legal protections implemented through procedures but not offer cryptographic protection. \textblue{Should add more...}
