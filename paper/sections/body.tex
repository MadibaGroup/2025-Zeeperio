...
At this point, the verifier has a polynomial commitment to $\mathsf{Mark}(X)$ which we will call $K_{\mathsf{Mark}(X)}$. How does the prover demonstrate it is binary? The prover will construct another polynomial as follows: $V(X)\ :=\ \mathsf{Mark}(X)\bigl(\mathsf{Mark}(X)-1\bigr)$. It will commit to $V(X)$ and send $K_{V(X)}$ to the verifier. If $\mathsf{Mark}(X)$ is binary for all $X\in H$, then $V(X)$ will be zero for all $X\in H$. If $\mathsf{Mark}(X)$ is not binary for at least one $X\in H$, $V(X)$ will be non-zero at the same $X$. We have converted the constraint $\mathsf{Mark}(X)$ is binary into: 
...
Across all the constraints, we will use an additional 5 `helper' polynomials. We will introduce them as needed, however they consist of 3 accumulator polynomials: $\mathsf{Acc}_{\mathsf{Audit}}(X)$,$\mathsf{Acc}_{\mathsf{Mark}}(X)$, and $\mathsf{Acc}_{\mathsf{Vote}}(X)$. As well as two block sum helper polynomials: $\mathsf{BlkSum}_{\mathsf{Audit}}(X)=\sum_{k=0}^{\mathcal{C}-1}\mathsf{Audit}(\omega^k X)$ and $\mathsf{BlkSum}_{\mathsf{Mark}}(X)=\sum_{k=0}^{\mathcal{C}-1}\mathsf{Mark}(\omega^k X)$.
