...
on the key $U$, and that each published table satisfies its prescribed shape constraints.

In Zeeperio, acceptance of $\Pi_{\mathrm{main}}$ implies the committed columns are consistent with a single induced table $E^Z$. 
$\mathcal{P}_{\mathrm{auth}}^Z$ is obtained by projecting $E^Z$ onto the published columns, so the merge
$\owedge^{\{U\}}(\mathcal{P}_{\mathrm{auth}}^Z)$ is non-empty. 
